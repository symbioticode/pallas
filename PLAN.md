...
> | `PALLAS-M18` | Honnêteté statistique : taille effective `appliedOrderSize = min(notional demandé, suggested_size_usd)/price` appliquée à l'émission (payload signé = appliedSize), `execution_success` journalise `applied_size`/`demanded_size`, Kelly `win_probability`/`odds` explicitement illustratifs (`warnKellyIllustrative` + commentaire) | ✅ clôturée 2026-09-11 |
> | `PALLAS-M19` | Custody credentials : `assertFilePermissions(0600)` appliqué au chargement (`SecretFilePermissionsError`), chemin fichier `loadPolymarketSecretsFromFile`, séparation/signature-dérivation documentée comme absente (risque nommé), rotation exécutée sur credentials de test, runbook `docs/RUNBOOK-key-compromise.md` | ✅ clôturée 2026-09-11 |
> | `PALLAS-M20` | Observabilité + CI durcie : alertes CRITICAL JSONL/webhook (`emitAnomaly`, 5 anomalies audit) câblées dans le loop (ambigu, réconciliations, kill switch, corruption state/ledger), endpoint `/api/status` + pavé ALERTS dashboard (frontière READ-ONLY), RTO/RPO dans SECURITY.md, SHA-pinning toutes actions CI, seuils de couverture (80/75/85/80), skip sandbox explicite (`bwrapProbe`) | ✅ clôturée 2026-09-11 |
